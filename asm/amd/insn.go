...
// a relative jump to an address not covered by the sym - jump outside the function.
// FindExternalJump returns the destination address of the relative jump outside the function.
func FindExternalJump(ef *pfelf.File, sym *libpf.Symbol) (libpf.Address, error) {
	var (
		err   error
		inst  x86asm.Inst
		start = int64(sym.Address)
		rip   = start
		end   = int64(sym.Address) + int64(sym.Size)
		code  []byte
	)
	code, err = ef.VirtualMemory(rip, int(sym.Size), math.MaxInt)
	if err != nil {
		return 0, err
	}
	if int(sym.Size) != len(code) {
	...
	for len(code) > 0 {
		...
		if rel, ok := inst.Args[0].(x86asm.Rel); !ok {
			...
			dst := rip + int64(rel)
			if dst >= int64(sym.Address) && dst < end {
				continue
			}
