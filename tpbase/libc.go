...
func ExtractTSDInfoX64_64(code []byte) (TSDInfo, error) {
	...
	if ok {
		return TSDInfo{
			Offset:     int16(offset.ExtractedValueImm()),
			Multiplier: uint8(mul.ExtractedValueImm()),
			Indirect:   1,
		}, nil
	}
	expected = variable.Mem(
		variable.Add(
			...
			variable.Multiply(variable.ZeroExtend(key, 32), mul),
			offset,
		),
		8,
	)
