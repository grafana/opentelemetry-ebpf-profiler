	...
	for j := 0; j < 137; j++ {
		...
		case x86asm.MOV:
			if dst, ok := op.Args[0].(x86asm.Reg); ok {
				res := it.Regs.Get(dst)
				gsOffset := sym.Symbol("gs offset")
				offset := sym.Symbol("offset")
				if res.Eval(sym.Mem(sym.Add(sym.MemS(x86asm.GS, gsOffset), offset))) {
					res := offset.ExtractedValue - 2*8
					if res < 0 || res > math.MaxUint32 {
						return 0, fmt.Errorf("overflow") // todo better error
					}
