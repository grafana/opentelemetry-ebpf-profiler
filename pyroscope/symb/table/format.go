...
type rangeEntry struct {
	...
	fileOffset stringOffset // todo make these optional?
	lineTable  lineTableRef // todo make these optional?
	callFile   stringOffset // todo make these optional?
	callLine   uint64       // todo make these optional?
}
...
func readLineTableEntry4(buf []byte) ffi.LineTableEntry {
	o := binary.LittleEndian.Uint32(buf[0:])
	l := binary.LittleEndian.Uint32(buf[4:])
	return ffi.LineTableEntry{Offset: o, LineNumber: l}
}
func readLineTableEntry2(buf []byte) ffi.LineTableEntry {
	o := binary.LittleEndian.Uint16(buf[0:])
	l := binary.LittleEndian.Uint16(buf[2:])
	return ffi.LineTableEntry{Offset: uint32(o), LineNumber: uint32(l)}
}
