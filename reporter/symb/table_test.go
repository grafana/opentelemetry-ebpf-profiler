...
func TestLibc(t *testing.T) {
	...
	check := func(syms []string) {
		fmt.Printf("%v\n", syms)
		assert.NotEmpty(t, syms)
	}
	check(table.Lookup(0x11ba61, nil))
	check(table.Lookup(0x18833e, nil))
	check(table.Lookup(0x9ca94, nil))
	check(table.Lookup(0x129c3c, nil))
	check(table.Lookup(0x98d61, nil))
	check(table.Lookup(0x9bc7e, nil))
}
