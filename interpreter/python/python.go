...
func findInterpreterRanges(ef *pfelf.File, info *interpreter.LoaderInfo) ([]util.Range, error) {
	...
	interpRange, err := info.GetSymbolAsRange("_PyEval_EvalFrameDefault")
	if err != nil {
		if interpRange, err = info.GetSymbolAsRange("PyEval_EvalFrameEx"); err != nil {
			return nil, err
		}
		return []util.Range{interpRange}, nil
	}
	ranges, err := decodeInterpreterRanges(ef, interpRange.Start)
	if err != nil {
		return nil, err
	}
	return ranges, nil
