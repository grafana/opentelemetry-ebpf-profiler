...
func retrieveExecuteExJumpLabelAddressAmd64(code []byte, addrBase libpf.SymbolValue) (
	...
	it := amd.NewInterpreter(code)
	it.CodeAddress = sym.Imm(uint64(addrBase))
	_, err := it.Loop(func(op x86asm.Inst) bool {
...
func retrieveJITBufferPtrAmd64(code []byte, addrBase libpf.SymbolValue) (
	...
	rdi := sym.Symbol("rdi")
	rsi := sym.Symbol("rsi")
	if it.Regs.Get(x86asm.RDI).Eval(sym.Mem(rdi)) && it.Regs.Get(x86asm.RSI).Eval(sym.Mem(rsi)) {
		return libpf.SymbolValue(rdi.ExtractedValue), libpf.SymbolValue(rsi.ExtractedValue), nil
	}
	return libpf.SymbolValueInvalid, libpf.SymbolValueInvalid,
		fmt.Errorf("failed to recover jit buffer: %s", "target not found")
}

func retrieveZendVMKindArm64(code []byte) (uint, error) {
...
func retrieveExecuteExJumpLabelAddressArm64(
...
func retrieveJITBufferPtrArm64(code []byte, addrBase libpf.SymbolValue) (
	dasmBuf libpf.SymbolValue, dasmSize libpf.SymbolValue, err error) {
...
func retrieveZendVMKindWrapper(code []byte) (uint, error) {
	...
	case "arm64":
		return retrieveZendVMKindArm64(code)
	case "amd64":
		return retrieveZendVMKindAmd64(code)
	default:
...
func retrieveJITBufferPtrWrapper(code []byte, addrBase libpf.SymbolValue) (
	...
	switch runtime.GOARCH {
	case "arm64":
		return retrieveJITBufferPtrArm64(code, addrBase)
	case "amd64":
		return retrieveJITBufferPtrAmd64(code, addrBase)
	default:
		return 0, 0, fmt.Errorf("unsupported arch %s", runtime.GOARCH)
	}
}
...
func retrieveExecuteExJumpLabelAddressWrapper(code []byte, addrBase libpf.SymbolValue) (
	...
	switch runtime.GOARCH {
	case "arm64":
		return retrieveExecuteExJumpLabelAddressArm64(code, addrBase)
	case "amd64":
		return retrieveExecuteExJumpLabelAddressAmd64(code, addrBase)
	default:
		return 0, fmt.Errorf("unsupported arch %s", runtime.GOARCH)
	}
